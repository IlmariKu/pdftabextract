...
def parse_pages(
    root, load_page_nums=None, require_image=False, only_load_topleft_image=True
):
    """
    ...
    for p in root.findall("page"):  # parse all pages
        ...
        if p_images:
            if len(p_images) == 1:
                ...
                if not only_load_topleft_image:
                    raise ValueError(
                        "multiple images on page %d but only_load_topleft_image was set to False"
                        % p_num
                    )
                for imgtag in p_images:
                    if (
                        int(imgtag.attrib["top"]) == 0
                        and int(imgtag.attrib["left"]) == 0
                    ):
            ...
            if require_image:
                raise ValueError(
                    "no image given on page %d but require_image was set to True"
                    % p_num
                )
            else:
                imgfile = None
        ...
        for t in p.findall("text"):
            ...
            last = len(page["texts"]) - 1

            # Nikke: small fix.
            # Swap item if added items dist from top is the same but left is smaller than prev elems
            while last:
                scd_last = last - 1
                last_item = page["texts"][last]
                scd_last_item = page["texts"][scd_last]

                if last_item["top"] != scd_last_item["top"]:
                    break

                elif last_item["left"] < scd_last_item["left"]:
                    page["texts"][last], page["texts"][scd_last] = (
                        page["texts"][scd_last],
                        page["texts"][last],
                    )
...
def create_text_dict(t, value=None):
    ...
    text = {"width": t_width, "height": t_height, "value": value, "xmlnode": t}

    update_text_dict_pos(
        text, pt(int(float(t.attrib["left"])), int(float(t.attrib["top"])))
    )
